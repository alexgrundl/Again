...
void PortAnnounceTransmit::TxAnnounce()
{
    ...
    portIdentity.portNumber = m_portGlobal->thisPort;
...
void PortAnnounceTransmit::ProcessState()
{
    ...
    else
    {
        ...
        case STATE_IDLE:
            if(m_timeAwareSystem->GetCurrentTime() >= m_announceSendTime && m_timeAwareSystem->selected[m_portGlobal->thisPort - 1] &&
                    !m_portGlobal->updtInfo)
            {
                m_portGlobal->newInfo = m_portGlobal->newInfo || m_timeAwareSystem->selectedRole[m_portGlobal->thisPort] == PORT_ROLE_MASTER;
                m_state = STATE_TRANSMIT_PERIODIC;
            }
            else if(m_portGlobal->newInfo && (m_timeAwareSystem->selectedRole[m_portGlobal->thisPort] == PORT_ROLE_MASTER) &&
                    (m_timeAwareSystem->GetCurrentTime() < m_announceSendTime) && m_timeAwareSystem->selected[m_portGlobal->thisPort - 1]
                    && !m_portGlobal->updtInfo)
            {
